...
public class InfoSubcommand {
	...
	private void displaySquadron(Squadron sq, CommandSender sender) {
		...
		if (sq.getCarrier() != null) {
			sender.sendMessage(">" + I18nSupport.getInternationalisedString("Carrier:" + ((sq.getCarrier().getName().toString() != "")?sq.getCarrier().getName():sq.getCarrier().getCraftProperties().getName())));
		}
		...
		for(Entry<SquadronCraft, Integer> entry : sortedCrafts)
			sender.sendMessage(" " + entry.getValue() + ":" + ((entry.getKey().getName().toString() != "")?entry.getKey().getName():entry.getKey().getCraftProperties().getName()) + "(" + entry.getKey().getHitBox().size() + ")");
